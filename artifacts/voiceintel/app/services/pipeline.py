...
def run_ingestion_pipeline(app):
    """Full pipeline: fetch emails → convert audio → transcribe → NLP → save to DB."""
    from app import db
    from app.models.voicemail import Voicemail, Transcript, Insight, Category
    from app.services import email_service, audio_service
    ...
    with app.app_context():
        ...
        for item in emails:
            try:
                ...
                if item.get("uid"):
                    try:
                        email_service.mark_email_read(item["uid"])
                    except Exception:
                        pass
